...
def test_LFNet(
    ...
    if path is not None:
        ...
        if save_path is None:
            save_path = path+'_With_IMsF_l%d_f%d/'%(crop_length,factor)
    ...
    for scene in sceneNameTuple:
        ...
        if save_results:
            ...
            bic_save_path = os.path.join(save_path,scene + '_BIC')
            ...
            if os.path.isdir(bic_save_path):
                log.info('-'*40)
                del_files(bic_save_path,'png')
                log.info('Bicubic path %s exists, delete all .png files' % bic_save_path)
            else:
                os.mkdir(bic_save_path)
        ...
        output_lf_r = np.zeros((x_res,y_res,s_res,t_res)).astype(config.floatX)
        output_lf_c = np.zeros((x_res,y_res,s_res,t_res)).astype(config.floatX)
        output_lf = np.zeros((x_res,y_res,s_res,t_res)).astype(config.floatX)


        # row_lf = bic_lf[np.newaxis,:,:,:,:].transpose((3,4,0,1,2))
        # col_lf = bic_lf[np.newaxis,:,:,:,:].transpose((4,3,0,1,2))
        log.info('='*40)
        s_time = time.time()
        ...
        for s_n in range(s_res):
            ...
            output_lf_r[:,:,s_n,:] = np.transpose(up_row_seq[:,0,0,:,:],(1,2,0))
        ...
        for t_n in range(t_res):
            ...
            output_lf_c[:,:,:,t_n] = np.transpose(up_col_seq[:,0,0,:,:],(1,2,0))
        # up_row_lf = f_x(row_lf)
        # up_col_lf = f_x(col_lf)
        output_lf = (output_lf_r + output_lf_c)/2.0
        process_time = time.time() - s_time
        log.info('Elapsed Time: %.2f sec.' % process_time)
        # output_lf = ( up_row_lf[:,:,0,:,:].transpose((2,3,0,1)) + up_col_lf[:,:,0,:,:].transpose((2,3,1,0)) )/2.0
        ...
        for s_n in xrange(s_res):
            for t_n in xrange(t_res):
                gt_img = lf[:,:,0,s_n,t_n]
                # print('GT shape %s' % str(gt_img.shape))
                # print('GT range: %.2f-%.2f' %(gt_img.min(),gt_img.max()))
                view_img_r = np.clip(output_lf_r[:,:,s_n,t_n],gt_img.min(),gt_img.max())
                view_img_c = np.clip(output_lf_c[:,:,s_n,t_n],gt_img.min(),gt_img.max())
                view_img = np.clip(output_lf[:,:,s_n,t_n],gt_img.min(),gt_img.max())
                # print('Predicted shape %s' % str(view_img.shape))
                bic_img = np.clip(bic_lf[:,:,s_n,t_n],gt_img.min(),gt_img.max())
                # print('LR shape %s' % str(bic_img.shape))
                ...
                this_PSNR_r = psnr(np.uint8(view_img_r*255.0),np.uint8(gt_img*255.0))
                this_SSIM_r = ssim(np.uint8(view_img_r*255.0),np.uint8(gt_img*255.0))
                this_PSNR_c = psnr(np.uint8(view_img_c*255.0),np.uint8(gt_img*255.0))
                this_SSIM_c = ssim(np.uint8(view_img_c*255.0),np.uint8(gt_img*255.0))
                ...
                log.info('Bicubic PSNR: %.2fdB SSIM: %.4f' %(bic_PSNR, bic_SSIM))
                PSNR.append(this_PSNR)
                SSIM.append(this_SSIM)
                PSNR_r.append(this_PSNR_r)
                SSIM_r.append(this_SSIM_r)
                PSNR_c.append(this_PSNR_c)
                SSIM_c.append(this_SSIM_c)
                ...
                if save_results:
                    ...
                    bicubicName = os.path.join(bic_save_path,'View_'+str(s_n+1)+'_'+str(t_n+1)+'.png')
                    out_img = np.zeros((x_res,y_res,3))
                    bicubic_img = np.zeros((x_res,y_res,3))
                    gt_out_img = np.zeros((x_res,y_res,3))

                    bicubic_img[:,:,0] = np.clip(bic_img*255.0,16.0,235.0)
                    ...
                    bicubic_img[:,:,1:3] = lf[:,:,1:3,s_n,t_n]*255.0
                    ...
                    bicubic_img = color.ycbcr2rgb(bicubic_img)
                    bicubic_img = np.clip(bicubic_img,0.0,1.0)
                    bicubic_img = np.uint8(bicubic_img*255.0)
                    ...
                    io.imsave(bicubicName,bicubic_img)
        ...
        log.info('-'*17+'Row'+'-'*17)
        log.info('[PSNR] Min: %.2f Avg: %.2f Max: %.2f dB' %(np.min(np.array(PSNR_r)),
                                                         np.mean(np.array(PSNR_r)),
                                                         np.max(np.array(PSNR_r))))
        log.info('[SSIM] Min: %.4f Avg: %.4f Max: %.4f' %(np.min(np.array(SSIM_r)),
                                                         np.mean(np.array(SSIM_r)),
                                                         np.max(np.array(SSIM_r))))
        log.info("[Elapsed time] %.2f sec" % process_time)
        log.info('-'*12+'Column'+'-'*12)
        log.info('[PSNR] Min: %.2f Avg: %.2f Max: %.2f dB' %(np.min(np.array(PSNR_c)),
                                                         np.mean(np.array(PSNR_c)),
                                                         np.max(np.array(PSNR_c))))
        log.info('[SSIM] Min: %.4f Avg: %.4f Max: %.4f' %(np.min(np.array(SSIM_c)),
                                                         np.mean(np.array(SSIM_c)),
                                                         np.max(np.array(SSIM_c))))
        log.info("[Elapsed time] %.2f sec" % process_time)
        log.info('-'*13+'Full'+'-'*13)
...
if __name__ == '__main__':
    ...
    test_LFNet(path=path,model_path=model_path,factor=factor, train_length=train_length,
               crop_length=crop_length, scene_names=scene_names, save_results=save_results)
