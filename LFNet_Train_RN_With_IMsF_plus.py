...
def opts_parser():
    ...
    usage = "LFNet RN Train With IMsF"
    ...
    parser.add_argument(
        '-M', dest='load_model_flag', action='store_true',
...
class LFNet(object):
    ...
    def build_net(self, model):
        ...
        cost = tensor.mean(tensor.flatten((y-proj)**2)) \
               # + 1e-6 * weight_decay
...
def train_LFNet(
    ...
    lrate=1e-1,  # Learning rate for sgd (not used for adadelta and rmsprop)
    ...
    print('='*40)
    print('... Loading data')
    train_path = 'LFNet_train_c1_s%d_l%d_f%d_p%d.mat' %(sample_num,length,factor,patch_size)
    options['train_path'] = train_path
    saveto='LFNet_RN_with_IMsF_f%d_l%d_plus' %(factor,length)  # The best model will be saved there
    options['savetoModel'] = saveto
    model_path='LFNet_RN_with_IMsF_f%d_l%d.npy' %(factor,length) # The model path
    ...
    try:
        for eidx in range(max_epochs):
            ...
            for train_index in kf[1]:
                ...
                x = theano.shared(value=x, borrow=True).dimshuffle(1, 0, 'x', 2, 3).eval()
                y = theano.shared(value=y, borrow=True).dimshuffle(1, 0, 'x', 2, 3).eval()
                ...
                if np.mod(uidx, validFreq) == 0:
                    ...
                    for valid_index in kv[1]:
                        ...
                        v_x = theano.shared(value=v_x, borrow=True).dimshuffle(1, 0, 'x', 2, 3).eval()
                        v_y = theano.shared(value=v_y, borrow=True).dimshuffle(1, 0, 'x', 2, 3).eval()
